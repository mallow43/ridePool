    ...
    geocoder.geocode(req.body.campus, function (err, data1) {
      if (err) {
        ...
        request(url, function (error, response, body) {
          console.log(error);
          if (error) {
            req.flash("error", error.error_message);
          ...
          if (!error && response.statusCode == 200) {
            ...
            Drive.create(drive, function (err, drive) {
              if (err) {
                console.log(err), req.flash("error", "Error adding drive.");
                res.redirect("back");
              } else {
  ...
  Calendar.findById(req.params.id)
    ...
    .exec(function (err, event) {
      if (err) {
        ...
        req.flash(
          "error",
          "Error adding you to the drive please try again later."
        );
        ...
        Drive.findById(event.drive._id, function (err, drive) {
          Pending.create(rider, function (err, rider) {
            if (err) {
              ...
              req.flash(
                "success",
                "Added Successfully, We will let you know when the driver approves your joining."
              );
              res.redirect("/calendar/" + req.params.id);
            }
          });
          ...
          User.findById(rider.userId, function (err, user) {
            if (err) {
              ...
              user.notifications.push({
                ...
                text:
                  "Sorry your request to join " +
                  event.drive.driver.firstName +
                  " " +
                  event.drive.driver.lastName +
                  "'s ride was declined.",
              });
              user.save();
              console.log(user);
            ...
            Pending.findByIdAndRemove(
              ...
              function (err, rider) {
                if (err) {
                  ...
                  req.flash(
                    "error",
                    "Error accepting this user try again later."
                  );
                  res.redirect("back");
                } else {
                  console.log("=====");
                  ...
                  Rider.create(rid, function (err, rider2) {
                    if (err) {
                      ...
                      req.user.notifications.push({
                        ...
                        text:
                          "Get ready for your carpool with " +
                          drive.driver.firstName +
                          " " +
                          drive.driver.lastName,
                        drive: drive._id,
                        event: event._id,
                      });
                      req.user.save();
                      console.log(drive);
                      drive.driveDistance.duration =
                        Number(drive.driveDistance.duration) +
                        Number(rider.duration);
                      drive.driveDistance.distance =
                        Number(drive.driveDistance.distance) +
                        Number(rider.distance);
                      console.log(drive);
                      drive.save();
                      console.log(drive);
  ...
  Calendar.findById(req.params.id)
    ...
    .exec(function (err, event) {
      var url1 =
        ...
        encodeURIComponent(event.drive.departure.location);
      ...
      var mailOptions = {
        ...
        html:
          "<!DOCTYPE html><head><link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/semantic-ui@2.4.2/dist/semantic.min.css'><style>.ui.button.black{color: #2185d0;margin-top: 10px;}h1{color:#2185d0}</style></head><body><div class='ui container'><h1><i class='code icon'></i>ridePool</h1><br><a href= '" +
          url1 +
          "'class='ui button black' style='text-align: center;'>Begin Rout in Google Maps</a></div></body></html>",
      };
